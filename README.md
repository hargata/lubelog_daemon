...
This should run within a private network where it can access your LubeLogger instance as well as any notification integrations you have configured, there is minimal security features for this service.

If the environment variable `LL_DAEMON_PASSWORD` is injected, it will require all POST requests to `/config` to have the password in the header under the value `x-daemon-password`
...
| hourToCheck         | UTC Hour of day to check for date-based reminders                                           | "5" means that it will check at 5 AM UTC everyday    |
| minuteToCheck       | UTC Minute of day to check for date-based reminders                                         | "5" means that it will check at 5:05 AM UTC everyday |
